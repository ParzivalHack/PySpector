<img width="1338" height="429" alt="image" src="https://github.com/user-attachments/assets/abf1349e-87c8-43ab-900a-6011792ec9fe" />

# PySpector Community Hackathon  
**20/12/2025 → 18/01/2026**

Benvenuto al primo hackathon ufficiale dedicato a **PySpector**, il SAST open-source community-driven sviluppato insieme dalla community di **SecurityCert**.

Questo evento è pensato per portare nuovi contributors, far crescere l’ecosistema dei plugin, migliorare le regole AST e premiare chi aiuta a far evolvere il progetto.

## 🎯 Obiettivi dell’Hackathon
Durante il periodo dell’evento potrai contribuire a PySpector in qualunque forma, seguendo le linee guida del nostro **CONTRIBUTING.md**.  
Ogni contributo valido genera punti: alla fine i partecipanti verranno classificati e **i primi 10 classificati, riceveranno dei premi speciali**.

# 🏆 Premi per i Top Contributors

## 🥇 **1° posto — Top Contributor dell’evento**
- **1 mese di Hack The Box VIP+**  
- **Mentorship 1-to-1 SecurityCert**  
- **1 mese di SatoriCI Pro**

## 🥈 **2° posto**
- **Mentorship 1-to-1 SecurityCert**
- **1 mese di Discord Nitro**   
- **1 mese di SatoriCI Pro**

## 🥉 **3° posto**
- **Mentorship 1-to-1 SecurityCert**
- **1 mese di SatoriCI Pro**

## 🏅 **4° → 10° posto**
- **1 mese di SatoriCI Pro**

⚠️Il numero minimo di partecipanti all'evento, per ritenerlo valido, è di **10 partecipanti singoli con contributions valide**

# 📊 Sistema di Punteggio

I punti si accumulano da tutti i contributi validi fatti tramite GitHub (issue, PR, plugin, vulnerability research, docs, etc...).  
Il punteggio viene assegnato come segue:

### ✅ **Issue**
- Issue valido, basic: **3 punti**
- Issue critico/di grande rilevanza: **5 punti**

### 🔧 **Pull Requests**
- PR valida (merged), basic: **5 punti**
- PR complessa/lunga/di grande impatto: **fino a 25 punti**

### 📚 **Documentazione**
- Aggiornamento/fix/miglioramento docs ufficiali: **3 punti**

### 🐞 **Vulnerabilità trovate in PySpector**
- Low / Informational: **1 punto**
- Medium: **5 punti**
- High: **15 punti**
- Critical: **30 punti**
- **RCE o Supply Chain Attack:** **50 punti**

### 🧩 **Nuovi Plugin (creati dalla community)**
- In base all’utilità, complessità e impatto: **da 15 a 40 punti**

# 🚀 Come Partecipare

Per partecipare ufficialmente servono **due requisiti obbligatori**:

1. ⭐ **Lasciare una stella al repo di PySpector**
   → https://github.com/ParzivalHack/PySpector

3. 💬 **Essere presenti nel server Discord di SecurityCert** (dove avverrà il tracking dei contributi, la leaderboard e il supporto)
   → https://discord.gg/KWzx6FCucF

Una volta completati questi due passaggi, potrai iniziare subito a contribuire :)


# 🛠 Come contribuire correttamente

Tutte le contributions, devono seguire le procedure definite nel file:

📄 **CONTRIBUTING.md**  
→ https://github.com/ParzivalHack/PySpector/blob/main/CONTRIBUTING.md

Questo include:
- apertura corretta delle issue  
- formattazione delle PR  
- standard di qualità del codice  
- struttura dei plugin  
- linee guida per reportare vulnerabilità interne

# 📡 Tracking dei Punti + Leaderboard

Durante l’evento:
- Ogni contributo verrà verificato manualmente dai maintainer
- I punti saranno aggiornati sul canale Discord dedicato alle News di PySpector
- La leaderboard sarà pubblica e aggiornata regolarmente
- Alla fine dell’evento verrà pubblicata la classifica definitiva
- Entro 7 giorni verranno consegnati tutti i premi

# 🤝 Sponsor Ufficiale

[**SatoriCI** – Automated Testing](https://satori.ci)  
Sponsor ufficiale del PySpector Hackathon 2025/2026.

# 🎉 Unisciti a noi!

L’hackathon è gratuito ed aperto a tutti: sviluppatori Python, Rust, security researchers, penetration tester, studenti e professionisti.  
Non serve essere esperti del progetto: basta voglia di imparare, contribuire e costruire il futuro di PySpector insieme alla community.

**Ti aspettiamo sul nostro server Discord!**  
→ https://discord.gg/KWzx6FCucF

# 📅 Date Ufficiali
- **Inizio:** 20 dicembre 2025  
- **Fine:** 18 gennaio 2026  

**Buona fortuna, e Happy Hacking!**
