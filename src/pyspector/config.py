...
def load_config(config_path: Path) -> dict:
    ...
    if config_path and config_path.exists():
        try:
            ...
            click.echo(
                click.style(
                    f"Warning: Could not parse config file '{config_path}'. Using defaults. Error: {e}",
                    fg="yellow",
                )
            )
...
def get_default_rules(ai_scan: bool = False) -> str:
    ...
    try:
        base_rules = (
            pkg_resources.files("pyspector.rules")
            .joinpath("built-in-rules.toml")
        ...
        if ai_scan:
            ...
            ai_rules = (
                pkg_resources.files("pyspector.rules")
                .joinpath("built-in-rules-ai.toml")
                .read_text(encoding="utf-8")
            )
